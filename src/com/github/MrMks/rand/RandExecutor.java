...
public class RandExecutor {
    ...
    public static RandResult execute(String[] args) {
    ...
    public static RandResult execute(String source) {
    ...
    private static Pattern pattern_l = Pattern.compile(long_pat);
    private static boolean longExecute(StringBuffer target) {
                    ...
                    try {
                        ...
                        if ((first == 0 || first == 1) && (second == 0 || second == 1) && (fourth == 0 || fourth == 1)) {
                            ...
                            if (t) {
                                ...
                                //target.replace(matcher.start(0), matcher.end(0), com.github.MrMks.rand.group.Group.getGroup(args[2]).getResult());
                                break;
                            }
                        }
                    } catch (Exception ignored) {
    ...
    private static Pattern pattern_s = Pattern.compile(short_pat);
    private static boolean shortExecute(StringBuffer target){
    ...
    private static void stringExecute(StringBuffer target, Matcher matcher, int returnType) {
        NumberResult result;
    ...
    public static NumberResult getResult(double min, double max, int returnType){
        max = setInRange(max);
        min = setInRange(min);
    ...
    private static double getDoubleResult(double min, double max) {
        return Math.random() * (max - min) + min;
    }

    private static long getLongResult(double min, double max) {
        return Math.round(Math.random() * (Math.floor(max) - Math.ceil(min)) + Math.ceil(min));
    }

    private static double setInRange(double number){
        double abs = Math.abs(number);
        return abs >= 30_000_000 ? (number/abs)*30_000_000 : number;
    }

    private static void replaceString(StringBuffer tar, Matcher matcher, String content){
        replaceString(tar,matcher.start(0),matcher.end(0),content);
    }

    private static void replaceString(StringBuffer tar, int start, int end, String content){
